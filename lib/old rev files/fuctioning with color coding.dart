...
class _SurahPageState extends State<SurahPage> {
  ...
  void _markAyahAsForgotten() {
    setState(() {
      ...
      // Add current ayah to forgotten set
      _forgottenAyahs[widget.pageNumber]!.add(_currentAyah);

      // Remove the ayah from fully revealed set to hide it
      _fullyRevealedAyahs.remove(_currentAyah - 1);
      // Also remove from partially revealed set if in word-by-word mode
      _partiallyRevealedAyahs.remove(_currentAyah - 1);

      print('_forgottenAyahs: ${_forgottenAyahs[widget.pageNumber]}');

      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Ayah marked for review'),
  ...
  void _resetForgottenAyahs() {
    setState(() {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              'Review list changed to ${_forgottenAyahs[widget.pageNumber]}'),
